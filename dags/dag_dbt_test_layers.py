...
# Deploying the silver layer
current_dag_id = 'test-dbt_layer_bronze'
...
with DAG(
	...
	dbt_deps = BashOperator(
		...
		bash_command=f'cd {DAGS_DIR} && dbt run --select tag:bronze'
	)

# Deploying the silver layer
current_dag_id = 'test-dbt_layer_silver'
current_dag_config = get_dag_config(current_dag_id)
...
with DAG(
	...
	dbt_deps = BashOperator(
		...
		bash_command=f'cd {DAGS_DIR} && dbt run --select tag:silver'
	)
 
# Deploying the silver layer
current_dag_id = 'test-dbt_layer_gold'
current_dag_config = get_dag_config(current_dag_id)
...
with DAG(
	...
	dbt_deps = BashOperator(
		...
		bash_command=f'cd {DAGS_DIR} && dbt run --select tag:gold'
	)
